...
class Teams extends Component {
    ...
    componentDidMount= () => {
      console.log(this.props);
        fetch('https://tp-react-2-6yahedvf3-hajar-2.vercel.app/api/videogames/'+this.props.id+'/teams?sort=&page='+this.state.currentPage+'&per_page='+this.state.LPerPage, this.state.options)
        .then(response =>  response.json())
        .then(response => {console.log(response); this.setState({teams:response});})
        .catch(err => console.error(err));

        fetch('https://tp-react-2-6yahedvf3-hajar-2.vercel.app/api/videogames/'+this.props.id+'/teams', this.state.options)
